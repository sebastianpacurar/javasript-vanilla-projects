...
$(document).ready(function () {
    ...
    $("#submit").click(function () {
        ...
        if (parseInt($firstNum.text()) + parseInt($secondNum.text()) === parseInt($answerNum.val())) {
            ...
            $answerEval.text(`Your answer is correct`).css({"color": "green", "font-size": "20px"});
            ...
            $answerEval.text(`Please start a new game`).css({"color": "blue", "font-size": "20px"});
        } else {

            // if answer is wrong then display your result and the actual result in red. fades out in 3 seconds
            $answerEval.text(`Your answer is ${$answerNum.val()} and the result is ` +
                `${parseInt($firstNum.text()) + parseInt($secondNum.text())}`)
